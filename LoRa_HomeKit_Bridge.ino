...
void displayStatus() {
 ...
 <title>LoRa HomeKit Bridge</title>
 <style>
 *{box-sizing:border-box}
 body{font-family:-apple-system,system-ui,sans-serif;background:linear-gradient(135deg,#1a1a2e 0%,#16213e 100%);color:#eee;margin:0;padding:20px;min-height:100vh}
 .container{max-width:500px;margin:0 auto}
 h1{color:#4ecdc4;text-align:center;margin-bottom:8px}
 .subtitle{text-align:center;color:#888;margin-bottom:24px}
 .card{background:rgba(255,255,255,0.05);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:24px;margin:16px 0}
 .card h2{margin:0 0 16px 0;color:#ff6b35;font-size:1.1em;display:flex;align-items:center;gap:8px}
 ...
 .btn{background:linear-gradient(135deg,#4ecdc4,#44a08d);color:#000;border:none;padding:14px 28px;border-radius:10px;cursor:pointer;font-size:1em;font-weight:600;width:100%;margin-top:8px;transition:transform 0.2s,box-shadow 0.2s}
 .btn:hover{transform:translateY(-2px);box-shadow:0 4px 20px rgba(78,205,196,0.4)}
 .btn:active{transform:translateY(0)}
 .btn-danger{background:linear-gradient(135deg,#e74c3c,#c0392b)}
 .btn-danger:hover{box-shadow:0 4px 20px rgba(231,76,60,0.4)}
 .btn-secondary{background:rgba(255,255,255,0.1);color:#fff}
 .btn-secondary:hover{background:rgba(255,255,255,0.2);box-shadow:none}
 input,select{width:100%;padding:14px;margin:8px 0 16px 0;border-radius:10px;border:1px solid rgba(255,255,255,0.2);background:rgba(0,0,0,0.3);color:#fff;font-size:1em}
 input:focus,select:focus{outline:none;border-color:#4ecdc4}
 label{display:block;color:#aaa;font-size:0.9em;margin-top:8px}
 .wifi-row{display:flex;gap:8px;align-items:flex-start}
 .wifi-row select{flex:1;min-width:0;margin:8px 0 16px 0}
 .wifi-row button{flex:0 0 80px;margin:8px 0 16px 0;padding:14px 10px}
 .test-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
 .test-grid button{padding:12px 8px;font-size:0.9em}
 .status-badge{display:inline-block;padding:4px 12px;border-radius:20px;font-size:0.85em}
 .status-ok{background:rgba(46,204,113,0.2);color:#2ecc71}
 .status-warn{background:rgba(241,196,15,0.2);color:#f1c40f}
 .device-item{background:rgba(0,0,0,0.2);border-radius:10px;padding:12px;margin:8px 0}
 .device-id{font-weight:600;color:#4ecdc4}
 .device-values{color:#aaa;font-size:0.9em;margin-top:4px}
 .help{font-size:0.85em;color:#666;margin-top:4px}
 .section-divider{border-top:1px solid rgba(255,255,255,0.1);margin:20px 0;padding-top:20px}
 </style></head><body><div class="container">
 <h1>LoRa HomeKit Bridge</h1>)rawliteral";
 
     if (ap_mode) {
         html += "<p class=\"subtitle\">Setup Mode - Configure your bridge</p>";
     } else {
         html += "<p class=\"subtitle\">Connected to " + String(wifi_ssid) + "</p>";
     }
     ...
     if (ap_mode) {
         html += "<tr><td>Mode</td><td><span class=\"status-badge status-warn\">Setup Mode</span></td></tr>";
         html += "<tr><td>AP Name</td><td>" + String(AP_SSID) + "</td></tr>";
         html += "<tr><td>AP IP</td><td>" + WiFi.softAPIP().toString() + "</td></tr>";
     } else {
         html += "<tr><td>WiFi</td><td><span class=\"status-badge status-ok\">Connected</span></td></tr>";
         html += "<tr><td>IP Address</td><td>" + WiFi.localIP().toString() + "</td></tr>";
         html += "<tr><td>Signal</td><td>" + String(WiFi.RSSI()) + " dBm</td></tr>";
     }
     
     html += "<tr><td>LoRa</td><td>" + String(lora_frequency, 1) + " MHz, SF" + String(lora_sf) + ", " + String(lora_bw/1000) + "kHz</td></tr>";
     html += "<tr><td>Devices</td><td>" + String(getActiveDeviceCount()) + "</td></tr>";
     html += "<tr><td>Packets</td><td>" + String(packets_received) + "</td></tr>";
     
     if (!ap_mode) {
         html += "<tr><td>HomeKit Code</td><td style=\"font-family:monospace;font-size:1.2em;color:#4ecdc4\">" + String(homekit_code_display) + "</td></tr>";
     }
     
     html += "</table></div>";
     
     // HomeKit Pairing Card (only in normal mode)
     if (!ap_mode) {
         bool isPaired = homeSpan.controllerListBegin() != homeSpan.controllerListEnd();
         
         html += "<div class=\"card\"><h2>HomeKit Pairing</h2>";
         
         // Pairing status
         html += "<div style=\"text-align:center;margin-bottom:16px\">";
         if (isPaired) {
             html += "<span class=\"status-badge status-ok\">✓ Paired with HomeKit</span>";
         } else {
             html += "<span class=\"status-badge status-warn\">Not paired - Add in Home app</span>";
         }
         html += "</div>";
         
         // HomeKit-style QR card (scalable design using container queries)
         html += "<style>";
         html += ".hk-card{width:260px;container-type:inline-size;display:flex;flex-direction:column;padding:10px;gap:10px;box-sizing:border-box;border:5px solid #000;border-radius:1cqi;background:#fff;font-family:monospace;line-height:1}";
         html += ".hk-top{display:flex;gap:2cqi;aspect-ratio:100/35}";
         html += ".hk-icon{height:100%;aspect-ratio:1}";
         html += ".hk-nums{flex:1;display:grid;grid-template-columns:repeat(4,1fr);align-content:space-between;font-weight:bold;font-size:16.5cqi;text-align:center;color:#000}";
         html += ".hk-qr{width:100%;height:100%;aspect-ratio:1;image-rendering:pixelated;display:block}";
         html += "</style>";
         
         html += "<div style=\"display:flex;justify-content:center;margin-bottom:16px\">";
         html += "<div class=\"hk-card\">";
         html += "<div class=\"hk-top\">";
         // Minified Apple HomeKit Icon
         html += "<svg class=\"hk-icon\" viewBox=\"0 0 29.8 26.2\"><path d=\"M3.5 23.6c0 1.7 1 2.6 2.7 2.6h17c1.8 0 2.8-1 2.8-2.6V9.5l-1.5-.2v14c0 .9-.5 1.4-1.4 1.4H6.4c-.9 0-1.4-.5-1.4-1.4V9.3l-1.5.2v14.1zM0 12.2c0 .4.3.7.8.7.2 0 .3 0 .5-.2L14.3 1.8c.3-.2.6-.3 1-.3l12.9 10.9c.1.1.3.2.5.2.5 0 .8-.3.8-.7 0-.3-.1-.5-.4-.7L16.1.6C15.3-.1 14.2-.1 13.3.6L.4 11.5c-.3.2-.4.4-.4.7zm23.1-5.3l2.9 2.4V3.6c0-.5-.3-.8-.8-.8H24c-.5 0-.8.3-.8.8v3.3z\" fill=\"#000\"/><path d=\"M7.6 20.6c0 1 .6 1.6 1.7 1.6h10.9c1 0 1.7-.6 1.7-1.6v-9.7l-6.4-5.3c-.5-.4-1.1-.4-1.6 0l-6.3 5.3v9.7zm1.5-.6v-8.3l5.2-4.4c.2-.2.5-.2.8 0l5.2 4.4v8.3c0 .4-.2.6-.7.6H9.8c-.5 0-.7-.2-.7-.6z\" fill=\"#000\"/><path d=\"M11.7 17c0 .7.4 1.1 1.1 1.1h3.8c.7 0 1.2-.4 1.2-1.1v-4l-2.4-2c-.4-.4-.9-.4-1.3 0l-2.4 2v4zm1.5-.7v-2.6l1.3-1.1c.2-.2.4-.2.5 0l1.3 1.1v2.7c0 .2-.1.3-.3.3h-2.6c-.1 0-.2-.1-.2-.3z\" fill=\"#000\"/></svg>";
         // Numbers grid
         html += "<div class=\"hk-nums\">";
         for (int i = 0; i < 8; i++) {
             html += "<div>" + String(homekit_code[i]) + "</div>";
         }
         html += "</div>";
         html += "</div>";
         // QR code
         ...
         html += "<p style=\"color:#888;font-size:0.9em;text-align:center\">Scan QR or enter code in Home app</p>";
         html += "<p style=\"color:#666;font-size:0.8em;text-align:center\">Setup ID: " HOMEKIT_SETUP_ID "</p>";
         
         // Unpair button (only if paired)
         if (isPaired) {
             html += "<div class=\"section-divider\"></div>";
             html += "<button class=\"btn btn-danger\" onclick=\"if(confirm('Unpair from HomeKit? You will need to re-add in Home app.')){fetch('/api/unpair').then(function(){alert('Unpairing... Device will restart.');location.reload();})}\">Unpair HomeKit</button>";
         }
         
         html += "</div>";
     }
     
     // Devices Card (only in normal mode)
     if (!ap_mode) {
         int activeCount = getActiveDeviceCount();
         html += "<div class=\"card\"><h2>Devices (" + String(activeCount) + ")</h2>";
         if (activeCount == 0) {
             html += "<p style=\"color:#666\">No devices yet. Add test devices below or wait for LoRa sensors.</p>";
         }
         for (int i = 0; i < device_count; i++) {
             if (!devices[i].active) continue;
             String devId = String(devices[i].id);
             String devName = String(devices[i].name);
             html += "<div class=\"device-item\">";
             html += "<div style=\"display:flex;justify-content:space-between;align-items:center\">";
             html += "<div class=\"device-id\">" + devName + "</div>";
             html += "<div style=\"display:flex;gap:4px\">";
             html += "<button class=\"btn btn-secondary\" style=\"padding:4px 8px;font-size:0.8em\" onclick=\"renameDevice('" + devId + "','" + devName + "')\">Rename</button>";
             html += "<button class=\"btn btn-danger\" style=\"padding:4px 8px;font-size:0.8em\" onclick=\"removeDevice('" + devId + "')\">Remove</button>";
             html += "</div></div>";
             if (strcmp(devices[i].id, devices[i].name) != 0) {
                 html += "<div style=\"color:#666;font-size:0.8em\">ID: " + devId + "</div>";
             }
             html += "<div class=\"device-values\">";
             if (devices[i].has_temp) html += "T:" + String(devices[i].temperature, 1) + "C ";
             if (devices[i].has_hum) html += "H:" + String((int)devices[i].humidity) + "% ";
             if (devices[i].has_batt) html += "B:" + String(devices[i].battery) + "% ";
             if (devices[i].has_light) html += "L:" + String(devices[i].lux) + "lx ";
             if (devices[i].has_motion) html += (devices[i].motion ? "Motion! " : "No motion ");
             if (devices[i].has_contact) html += (devices[i].contact ? "Open " : "Closed ");
             html += "RSSI:" + String(devices[i].rssi) + "dBm";
             html += "</div>";
             // Sensor type selectors (only show if device has that capability)
             if (devices[i].has_contact || devices[i].has_motion) {
                 html += "<div style=\"margin-top:8px;display:flex;gap:8px;flex-wrap:wrap\">";
                 if (devices[i].has_contact) {
                     html += "<select style=\"flex:1;min-width:120px;padding:6px;font-size:0.85em\" onchange=\"setSensorType('" + devId + "','contact',this.value)\">";
                     html += "<option value=\"0\"" + String(devices[i].contact_type == 0 ? " selected" : "") + ">Contact</option>";
                     html += "<option value=\"1\"" + String(devices[i].contact_type == 1 ? " selected" : "") + ">Leak ⚠️</option>";
                     html += "<option value=\"2\"" + String(devices[i].contact_type == 2 ? " selected" : "") + ">Smoke ⚠️</option>";
                     html += "<option value=\"3\"" + String(devices[i].contact_type == 3 ? " selected" : "") + ">CO ⚠️</option>";
                     html += "<option value=\"4\"" + String(devices[i].contact_type == 4 ? " selected" : "") + ">Occupancy</option>";
                     html += "</select>";
                 }
                 if (devices[i].has_motion) {
                     html += "<select style=\"flex:1;min-width:120px;padding:6px;font-size:0.85em\" onchange=\"setSensorType('" + devId + "','motion',this.value)\">";
                     html += "<option value=\"0\"" + String(devices[i].motion_type == 0 ? " selected" : "") + ">Motion</option>";
                     html += "<option value=\"1\"" + String(devices[i].motion_type == 1 ? " selected" : "") + ">Occupancy</option>";
                     html += "<option value=\"2\"" + String(devices[i].motion_type == 2 ? " selected" : "") + ">Leak ⚠️</option>";
                     html += "<option value=\"3\"" + String(devices[i].motion_type == 3 ? " selected" : "") + ">Smoke ⚠️</option>";
                     html += "<option value=\"4\"" + String(devices[i].motion_type == 4 ? " selected" : "") + ">CO ⚠️</option>";
                     html += "</select>";
                 }
                 html += "</div>";
             }
     ...
     // Test Devices Card (only in normal mode when HomeKit is running)
     if (!ap_mode && homekit_started) {
         html += "<div class=\"card\"><h2>Test Devices</h2>";
         html += "<p style=\"color:#888;font-size:0.9em;margin-bottom:16px\">Add simulated sensors to test HomeKit integration</p>";
         html += "<div class=\"test-grid\">";
         html += "<button class=\"btn btn-secondary\" onclick=\"addTest('temp')\">Temperature</button>";
         html += "<button class=\"btn btn-secondary\" onclick=\"addTest('humidity')\">Humidity</button>";
         html += "<button class=\"btn btn-secondary\" onclick=\"addTest('temp_hum')\">Temp + Humidity</button>";
         html += "<button class=\"btn btn-secondary\" onclick=\"addTest('motion')\">Motion</button>";
         html += "<button class=\"btn btn-secondary\" onclick=\"addTest('contact')\">Contact</button>";
         html += "<button class=\"btn btn-secondary\" onclick=\"addTest('light')\">Light Level</button>";
         html += "<button class=\"btn btn-secondary\" onclick=\"addTest('full')\">Full Sensor</button>";
         html += "<button class=\"btn btn-secondary\" onclick=\"addTest('update')\">Update Values</button>";
         html += "</div>";
         html += "<p id=\"test-status\" style=\"color:#4ecdc4;font-size:0.9em;margin-top:12px;text-align:center\"></p>";
         html += "</div>";
     }
     
     // Settings Card - build without raw literals to avoid escaping issues
     html += "<div class=\"card\"><h2>Settings</h2>";
     html += "<form method=\"POST\" action=\"/save\">";
     html += "<label>WiFi Network</label>";
     html += "<div class=\"wifi-row\">";
     html += "<select id=\"wifi_select\" onchange=\"document.getElementById('ssid').value=this.value\">";
     html += "<option value=\"\">-- Scan for networks --</option>";
     html += "</select>";
     html += "<button type=\"button\" class=\"btn btn-secondary\" onclick=\"scanWiFi()\">Scan</button>";
     html += "</div>";
     html += "<input type=\"text\" id=\"ssid\" name=\"ssid\" placeholder=\"WiFi SSID\" value=\"" + String(wifi_ssid) + "\" required>";
     
     html += "<label>WiFi Password</label>";
     String pwPlaceholder = strlen(wifi_password) > 0 ? "Leave empty to keep current" : "Enter password";
     html += "<input type=\"password\" name=\"password\" placeholder=\"" + pwPlaceholder + "\">";
     
     html += "<div class=\"section-divider\"></div>";
     
     // Warning box
     html += "<div style=\"background:rgba(231,76,60,0.2);border:1px solid #e74c3c;border-radius:8px;padding:12px;margin-bottom:16px\">";
     html += "<p style=\"color:#e74c3c;margin:0;font-size:0.9em\">⚠️ <strong>LoRa &amp; Encryption settings must match your sensors!</strong> Mismatched settings will prevent communication.</p>";
     html += "</div>";
     
     html += "<label>LoRa Frequency</label>";
     html += "<select name=\"freq\">";
     html += "<option value=\"433.0\"" + String(lora_frequency < 500 ? " selected" : "") + ">433.0 MHz (Asia)</option>";
     html += "<option value=\"868.0\"" + String(lora_frequency > 800 && lora_frequency < 900 ? " selected" : "") + ">868.0 MHz (Europe)</option>";
     html += "<option value=\"915.0\"" + String(lora_frequency > 900 ? " selected" : "") + ">915.0 MHz (Americas)</option>";
     html += "</select>";
     
     html += "<label>Spreading Factor (SF)</label>";
     html += "<select name=\"lora_sf\">";
     for (int sf = 6; sf <= 12; sf++) {
         html += "<option value=\"" + String(sf) + "\"" + String(lora_sf == sf ? " selected" : "") + ">SF" + String(sf);
         if (sf == 7) html += " (fast)";
         if (sf == 12) html += " (long range)";
         html += "</option>";
     }
     html += "</select>";
     html += "<div class=\"help\">Higher SF = longer range but slower</div>";
     
     html += "<label>Bandwidth</label>";
     html += "<select name=\"lora_bw\">";
     html += "<option value=\"125000\"" + String(lora_bw == 125000 ? " selected" : "") + ">125 kHz (standard)</option>";
     html += "<option value=\"250000\"" + String(lora_bw == 250000 ? " selected" : "") + ">250 kHz</option>";
     html += "<option value=\"500000\"" + String(lora_bw == 500000 ? " selected" : "") + ">500 kHz (fast)</option>";
     html += "</select>";
     
     html += "<label>Coding Rate</label>";
     html += "<select name=\"lora_cr\">";
     html += "<option value=\"5\"" + String(lora_cr == 5 ? " selected" : "") + ">4/5 (standard)</option>";
     html += "<option value=\"6\"" + String(lora_cr == 6 ? " selected" : "") + ">4/6</option>";
     html += "<option value=\"7\"" + String(lora_cr == 7 ? " selected" : "") + ">4/7</option>";
     html += "<option value=\"8\"" + String(lora_cr == 8 ? " selected" : "") + ">4/8 (max redundancy)</option>";
     html += "</select>";
     
     html += "<label>Preamble Length</label>";
     html += "<input type=\"number\" name=\"lora_pre\" value=\"" + String(lora_preamble) + "\" min=\"6\" max=\"65535\">";
     html += "<div class=\"help\">Default: 6-8, higher values improve detection</div>";
     
     html += "<label>Sync Word (hex)</label>";
     char syncHex[5];
     sprintf(syncHex, "%02X", lora_syncword);
     html += "<input type=\"text\" name=\"lora_sync\" value=\"" + String(syncHex) + "\" maxlength=\"2\" pattern=\"[0-9A-Fa-f]{2}\">";
     html += "<div class=\"help\">Default: 12 (must match sensors)</div>";
     
     html += "<div class=\"section-divider\"></div>";
     
     html += "<label>Gateway Key</label>";
     html += "<input type=\"text\" name=\"gw_key\" value=\"" + String(gateway_key) + "\" placeholder=\"Must match your sensors\">";
     html += "<div class=\"help\">Sensors with different keys will be ignored</div>";
     
     html += "<label>Encryption Mode</label>";
     html += "<select name=\"enc_mode\">";
     html += "<option value=\"0\"" + String(encryption_mode == ENCRYPT_NONE ? " selected" : "") + ">None</option>";
     html += "<option value=\"1\"" + String(encryption_mode == ENCRYPT_XOR ? " selected" : "") + ">XOR</option>";
     html += "<option value=\"2\"" + String(encryption_mode == ENCRYPT_AES ? " selected" : "") + ">AES (ESP-NOW)</option>";
     html += "</select>";
     html += "<div class=\"help\">Must match your sensors encryption method</div>";
     
     html += "<label>Encryption Key (hex)</label>";
     html += "<input type=\"text\" name=\"enc_key\" value=\"";
     
     // Add current encryption key as hex
     ...
     html += "\" pattern=\"[0-9A-Fa-f]*\" placeholder=\"e.g., 4BA33F9C\">";
     html += "<div class=\"help\">XOR: 2-32 hex chars, AES: exactly 32 hex chars (16 bytes)</div>";
     
     html += "<button type=\"submit\" class=\"btn\">Save and Restart</button>";
     html += "</form>";
     
     html += "<div class=\"section-divider\"></div>";
     
     html += "<button class=\"btn btn-secondary\" onclick=\"if(confirm('Restart device?')){fetch('/api/restart').then(function(){alert('Restarting...');setTimeout(function(){location.reload();},3000);})}\">Restart Device</button>";
     
     html += "<div class=\"section-divider\"></div>";
     
     html += "<form method=\"POST\" action=\"/reset\" onsubmit=\"return confirm('Reset all settings to defaults?')\">";
     ...
     html += "var sel=document.getElementById('wifi_select');";
     html += "sel.innerHTML='<option>Scanning...</option>';";
     html += "fetch('/api/scan').then(function(r){return r.json();}).then(function(data){";
     html += "sel.innerHTML='<option value=\"\">-- Select network --</option>';";
     html += "data.networks.sort(function(a,b){return b.rssi-a.rssi;}).forEach(function(n){";
     html += "var sig=n.rssi>-50?'Strong':n.rssi>-70?'Good':'Weak';";
     html += "var lock=n.secure?' [secure]':'';";
     html += "sel.innerHTML+='<option value=\"'+n.ssid+'\">'+sig+': '+n.ssid+' ('+n.rssi+' dBm)'+lock+'</option>';";
     html += "});";
     html += "}).catch(function(){sel.innerHTML='<option value=\"\">Scan failed</option>';});";
     html += "}";
     
     // Add test device function
     html += "function addTest(type){";
     html += "var status=document.getElementById('test-status');";
     html += "if(status)status.innerHTML='Adding '+type+' sensor...';";
     html += "fetch('/api/test?type='+type).then(function(r){return r.json();}).then(function(data){";
     html += "if(status)status.innerHTML=data.message;";
     html += "setTimeout(function(){location.reload();},2000);";
     html += "}).catch(function(e){";
     html += "if(status)status.innerHTML='Error: '+e;";
     html += "});";
     html += "}";
     
     // Rename device function
     html += "function renameDevice(id,currentName){";
     html += "var newName=prompt('Enter new name for '+id+':',currentName);";
     html += "if(newName && newName!==currentName){";
     html += "fetch('/api/rename?id='+encodeURIComponent(id)+'&name='+encodeURIComponent(newName))";
     html += ".then(function(r){return r.json();})";
     html += ".then(function(data){alert(data.message);location.reload();})";
     html += ".catch(function(e){alert('Error: '+e);});";
     html += "}";
     html += "}";
     
     // Remove device function
     html += "function removeDevice(id){";
     html += "if(confirm('Remove device '+id+'?')){";
     html += "fetch('/api/remove?id='+encodeURIComponent(id))";
     html += ".then(function(r){return r.json();})";
     html += ".then(function(data){alert(data.message);location.reload();})";
     html += ".catch(function(e){alert('Error: '+e);});";
     html += "}";
     ...
     // Set sensor type function
     html += "function setSensorType(id,sensor,type){";
     html += "fetch('/api/settype?id='+encodeURIComponent(id)+'&sensor='+sensor+'&type='+type)";
     html += ".then(function(r){return r.json();})";
     html += ".then(function(data){";
     html += "if(data.success){alert(data.message);location.reload();}";
     html += "else{alert('Error: '+data.message);}";
     html += "}).catch(function(e){alert('Error: '+e);});";
     html += "}";
     
     // Generate HomeKit QR code on page load
     // HomeKit Setup Payload format: X-HM://[base36 encoded flags+category+setupcode][4-char setup ID]
     // For Bridge (cat=2) with IP (flags=2) and code 466-37-726:
     // Payload = (2<<31)|(2<<27)|46637726 = 4610040478, base36 = "2K4TQ5E", padded = "002K4TQ5E"
     html += "window.onload=function(){";
     html += "var qrDiv=document.getElementById('qrcode');";
     html += "if(qrDiv && typeof qrcode !== 'undefined'){";
     html += "try{";
     html += "var qr=qrcode(0,'M');";
     html += "qr.addData('" + String(homekit_qr_uri) + "');";
     html += "qr.make();";
     html += "var img=qr.createImgTag(4,0);";
     html += "qrDiv.innerHTML=img;";
     html += "qrDiv.querySelector('img').className='hk-qr';";
     html += "}catch(e){qrDiv.innerHTML='<p style=\"color:#666\">QR unavailable</p>';}";
     html += "}";
     html += "};";
