...
void loadSettings() {
  ...
  wifi_configured =
      prefs.isKey("wifi_ssid") && prefs.getString("wifi_ssid", "").length() > 0;
  ...
  Serial.printf("[SETTINGS] Loaded - WiFi: %s, Freq: %.1f MHz, SF%d, BW:%dkHz, "
                "Code: %s\n",
                wifi_configured ? "YES" : "NO", lora_frequency, lora_sf,
                lora_bw / 1000, homekit_code_display);
}
...
void loadDevices() {
  ...
  for (int i = 0; i < saved_count && i < MAX_DEVICES; i++) {
    ...
    Serial.printf("[DEVICES] Loaded: %s (%s) ctype:%d mtype:%d\n", dev->id,
                  dev->name, dev->contact_type, dev->motion_type);
  }
